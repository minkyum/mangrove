...
# Point Shape file
utm_crs = imgBase@crs
geog <- CRS("+proj=longlat +datum=WGS84")
site <- data.frame(1,cx,cy)
colnames(site) <- c('id','xcrd','ycrd')
xy   <- site[,c(2,3)]
bb   <- SpatialPointsDataFrame(coords=xy,data=site,proj4string=geog)
...
######################################################
## GCC
gcc <- read.csv('/projectnb/planet/PhenoCam/PhenoCam_V3/data_record_4/ibp_GR_1000_3day.csv',skip=24)
pcDat <- gcc[,c(1,21)]
pcDat <- pcDat[1141:2966,]
pcDat1 <- na.omit(pcDat)

gcc <- read.csv('/projectnb/planet/PhenoCam/PhenoCam_V3/data_record_4/ibp_SH_1000_3day.csv',skip=24)
pcDat <- gcc[,c(1,21)]
pcDat <- pcDat[1069:2894,]
pcDat2 <- na.omit(pcDat)


###
setwd('/projectnb/modislc/users/mkmoon/Planet/data_paper/figure/')
png(filename='pt_ts_jornada_gcc_1.png',width=13,height=6,unit='in',res=600)
...
# abline(v=as.Date('2017-8-5'),lty=5,lwd=2)
# abline(v=as.Date('2018-7-5'),lty=5,lwd=2)
# abline(v=as.Date('2021-8-15'),lty=5,lwd=2)
# text(as.Date('2017-8-15'),0.28,'L',font=2,pos=2,cex=2)
# text(as.Date('2018-8-15'),0.28,'M',font=2,pos=2,cex=2)
# text(as.Date('2021-8-15'),0.28,'R',font=2,pos=2,cex=2)

# dev.off()

# points(dates,VI,ylim=c(0,0.42),pch=19,col=rgb(1,0,0,0.5),cex=0.6)
legend(17180,0.31,c('Planet','GCC_SH','GCC_GR'),pch=19,cex=1.7,
       col=c('black','red','forestgreen'),
       bty='n',pt.cex=c(0.8,1.1,1.1))

par(new = TRUE)
plot(as.Date(pcDat1[,1]),pcDat1[,2],
     axes=F,bty="n",xlab="",ylab="",
     pch=19,cex=1.1,col=alpha('forestgreen',0.6),
     ylim=c(0.30,0.47))
axis(4,seq(0,1,0.03),cex.axis=1.5)
mtext(expression(italic(G[CC])),4,3,cex=1.5)

points(as.Date(pcDat2[,1]),pcDat2[,2],
     pch=19,cex=1.1,col=alpha('red',0.6))
...
######################################################
rast1 <- raster('/projectnb/planet/PLSP/Product_netCDF/US-xJR__NEON_Jornada_LTER/PLSP_2017.nc',varname='Peak')
rast2 <- raster('/projectnb/planet/PLSP/Product_netCDF/US-xJR__NEON_Jornada_LTER/PLSP_2018.nc',varname='Peak')
rast3 <- raster('/projectnb/planet/PLSP/Product_netCDF/US-xJR__NEON_Jornada_LTER/PLSP_2021.nc',varname='Peak')
sum(!is.na(values(rast1)))/length(rast1)*100
sum(!is.na(values(rast2)))/length(rast2)*100
sum(!is.na(values(rast3)))/length(rast3)*100
