...
########################################
## 
rastPhe   <- raster('/projectnb/modislc/users/mkmoon/cdsa/plsp/ImgCliped/cdsa_toolik/phe/2020/07_2020_50PCGD.tif')
rastLctHi <- raster('/projectnb/modislc/users/mkmoon/cdsa/plsp/Vegetation_Maps_Toolik_Lake_1690/data/Toolik_veg_class.tif')
compareCRS(rastPhe,rastLctHi)
plot(rastPhe)
plot(rastLctHi,add=T)

rastPheCrop <- raster::crop(rastPhe,rastLctHi)
rastPheMask <- raster::mask(rastPheCrop,rastLctHi)

nn <- sprintf('%03d',numSite)
ptShp <- shapefile(paste0('/projectnb/modislc/users/mkmoon/cdsa/plsp/shp/pt_',numSite,'.shp'))
...
png(filename=paste0('ts_',strSite,'_eg.png'),width=6.5,height=6.5,unit='in',res=300)

par(mfrow=c(4,1),oma=c(0,1,0,0),mar=c(2,4,1,1),mgp=c(2.0,1,0))
for(ppp in 1:4){
  pixNum  <- pixNums[ppp]
  ...
  #   
  #   # plot(dateSub,viSub)
  #   points(pred_dates,smoothed_vi,type='l')
  #   
